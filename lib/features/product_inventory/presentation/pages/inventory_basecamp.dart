...
class InventoryBasecampPage extends StatefulWidget {
  const InventoryBasecampPage({super.key});

  @override
  State<InventoryBasecampPage> createState() => _InventoryBasecampPageState();
}

class _InventoryBasecampPageState extends State<InventoryBasecampPage> {
  ...
  Future<void> _loadInventory() async {
    ...
    setState(() {
      _futureInventory = getInventoryByLocation('Basecamp', token);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Inventory Produk — Basecamp'),
...
class _BatchDetailSheetState extends State<_BatchDetailSheet> {
  ...
  void _loadBatches() {
    ...
    print('Product ID Type: ${widget.item.productId.runtimeType}');
    print('Product Name: ${widget.item.name}');
    print('Token: ${widget.token.substring(0, 20)}...');
    print('Full Token: ${widget.token}'); // ← Cek token lengkap
